...
|------|------------|---------|
| Componente (atom/molecule) | `lowercase` shadcn-style o `PascalCase` | `button.jsx`, `timeline-entry.jsx` |
| Componente (organism/page) | `PascalCase` | `Navbar.jsx`, `Portfolio.jsx` |
| Hook | `camelCase`, prefijo `use`, **un hook por archivo** | `useActiveSection.js` |
...
> **Regla clave:** el nombre del archivo debe reflejar su export principal. Un archivo `useActiveSection.js` no debe exportar también otro hook: cada hook va en su propio archivo, descubrible por su nombre.

## Símbolos en código

- **Componentes:** `PascalCase` (`function Hero()`).
- **Hooks:** `camelCase` con `use` (`useActiveSection`).
...
- Sub-grupos para textos relacionados: `contact.labels.*`, `contact.socialLabels.*`.
...
- Clases globales reutilizables en `index.css` con nombres semánticos en `kebab-case` (`container-narrow`, `section-grid`, `skip-link`).
